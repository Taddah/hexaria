...
export class CoastGenerator {
    private COAST_TYPE: Record<number, TileType> = {
        1: TileType.COAST_1,
        2: TileType.COAST_2,
        3: TileType.COAST_3,
        4: TileType.COAST_4,
    ...
    private COAST_ROTATION_OFFSET: Record<number, number> = {
        1: 1,
        2: 2,
        3: 2,
        4: 2,
    ...
    generate(tiles: TileData[]): TileData[] {
        ...
        for (const tile of tiles) {
            ...
            const { start, count, isContiguous } = this.findConsecutiveWaterGroup(waterDirs);
            ...
            tile.type = this.COAST_TYPE[count] ?? TileType.COAST_4;
            if (tile.resource) delete tile.resource;
            tile.coastRotation = (start + (this.COAST_ROTATION_OFFSET[count] ?? 0)) % 6;
        }

        return tiles;
    }

    private findConsecutiveWaterGroup(waterDirs: number[]): {
        start: number;
        count: number;
